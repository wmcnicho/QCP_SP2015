...
The 2 main types of matrices we implemented were dense and sparse matrices. The dense matrix itself comes in 2 different types, real and complex. These 2 share some common features such as the fact that each element is explicitly stored even if it is a zero. This contrasts the sparse matrix as only non-zero elements and their indices are stored.
...
Our sparse matrix implementation contains both a general sparse matrix class as well as a specially optimised class designed for gates. In section 666.666 we saw that each gate can be represented by a matrix containing 2 elements in each column. This allowed us to design a data structure specifically tailored towards this problem.
