...
\end{align}
%
The resulting matrix $G_\text{tot}$ is in general a $2^N \times 2^N$-matrix with two and only two non-zero entries in very row and column. That means that only $2\cdot 2^N$ of $2^{N^2}$ entries are non-zero. The ratio is $2^{1-N}$ which rapidly goes towards zero for big $N$. This is a hint that a (dense) matrix representation for single qubit gates acting on a big quantum register is an incredibly inefficient way of storing gate data.
